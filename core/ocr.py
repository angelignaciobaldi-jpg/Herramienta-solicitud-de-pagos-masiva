"""Extracción de texto de documentos (carátulas bancarias, Vo.Bo., constancias).

Módulo REUTILIZABLE del estándar Quetzaltic: es el mismo `core/ocr.py` de la
Herramienta Integral de Tesorería, sin cambios de comportamiento. Lo que
distingue a esta herramienta vive en `core/adaptadores/ocr_caratula.py`, que
interpreta el texto que sale de aquí.
...
del equipo, que es un requisito de este proyecto tanto como del otro.
"""
...
# una instalación del sistema (útil en desarrollo). Fuera del PATH -> se apunta.
_RUTAS_TESSERACT = [
...
def _run_tesseract_oculto(args: list[str],
...
def tesseract_disponible() -> bool:
    """True si el binario de Tesseract responde. Se comprueba UNA vez y se
    cachea: evita relanzar tesseract (y su parpadeo de consola) en cada OCR."""
    ...
    if _tess_disponible is None:
        ...
        except Exception:  # noqa: BLE001 — binario ausente/ruta mala
...
def idioma_ocr() -> str:
    ...
    falla con rutas con acentos (p. ej. la carpeta 'innovación petroil').
    """
    global _tess_idioma
...
def _ocr_imagen(img: Image.Image, psm: int | None = None,
                cancelado=None) -> str:
    ...
    Devuelve el mismo texto que pytesseract (saltos normalizados a '\\n').
    """
    ...
    # pipe de stdout, cuyo buffer podría bloquear a tesseract si no se lee
    # mientras sondeamos la cancelación.
    ...
    try:
        ...
        while proc.poll() is None:
            if cancelado is not None and cancelado():
                ...
                raise OCRCancelado()
            time.sleep(0.05)
        with open(base_salida + ".txt", "r", encoding="utf-8",
                  errors="replace") as fh:
            texto = fh.read()
        # tesseract escribe LF en el .txt, pero se normaliza por si acaso, para
        # dar exactamente el mismo texto que daba pytesseract.
...
def ocr_imagen(img: Image.Image, psm: int | None = None, cancelado=None) -> str:
    """OCR de una imagen ya cargada y preprocesada por quien llama.

    Es la puerta pública a `_ocr_imagen` para los módulos de dominio que hacen
    su propio preprocesado (p. ej. `adaptadores/ocr_caratula.py`, que binariza y
    escala las fotos de carátula antes de pasarlas). Se expone para que no
    tengan que llamar a un privado y para que la cancelación —matar tesseract a
    media página— siga siendo la de este módulo.
    """
...
def _ocr_pagina(pagina: "fitz.Page", psm: int | None = None,
                cancelado=None) -> str:
    pix = pagina.get_pixmap(dpi=_DPI_OCR)
    # Rasterizar ya costó; antes de pagar el Tesseract de esta página, se mira
    # si entretanto pidieron cancelar. El propio _ocr_imagen también es
    # cancelable (mata tesseract si se detiene a media página).
    if cancelado is not None and cancelado():
        raise OCRCancelado()
    # Imagen directa desde los píxeles del pixmap (sin round-trip a PNG): evita
    # dos codificaciones/decodificaciones PNG por página (más rápido).
...
def _texto_pdf(ruta: str, forzar_ocr: bool = False, psm: int | None = None,
    ...
    correo de Outlook) y la carátula real está como imagen. Con psm se fuerza
    siempre OCR usando ese modo de segmentación de página.

    `cancelado` (opcional): se consulta ANTES de cada página; si devuelve True
    se lanza OCRCancelado y no se procesa el resto del documento.
    """
    partes: list[str] = []
    uso_ocr = False
...
def extraer_texto(ruta: str, forzar_ocr: bool = False, psm: int | None = None,
    ...
    if ext in (".png", ".jpg", ".jpeg", ".tif", ".tiff", ".bmp"):
        if cancelado is not None and cancelado():
            raise OCRCancelado(ruta)
        return _normalizar(
            _ocr_imagen(Image.open(ruta), psm=psm, cancelado=cancelado)), True
